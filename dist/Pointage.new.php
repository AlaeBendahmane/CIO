...
//   filename: 'Pointage_Export',
//   columnHeaders: true,
//   range: [0, 0, hot.countRows(), hot.countCols()]
// });
// 1. Get all data from Handsontable
async function exportWithStyles() {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Pointage');
    
    const { year, month } = getSelectedPeriod();
    const { headerRow1, headerRow2, numDays } = generateHeaders();
    const rowCount = hot.countRows();

    // --- 1. CONSTRUCTION DE LA LIGNE 1 (Headers avec Colspan) ---
    // On transforme les objets colspan en cellules individuelles pour Excel
    const row1Values = [];
    headerRow1.forEach(item => {
        if (typeof item === 'object') {
            row1Values.push(item.label);
            for (let i = 1; i < item.colspan; i++) row1Values.push(''); // Cellules vides pour le merge
        } else {
            row1Values.push(item);
        }
    });
    const excelRow1 = worksheet.addRow(row1Values);

    // --- 2. CONSTRUCTION DE LA LIGNE 2 (Lettres des jours et colonnes fixes) ---
    const excelRow2 = worksheet.addRow(headerRow2);

    // Style commun pour les headers
    ...
    excelRow1.eachCell(applyHeaderStyle);
    excelRow2.eachCell(applyHeaderStyle);

    // --- 3. FUSION DES CELLULES (Merge) ---
    // Merge pour "Agent" (colspan: 2)
    worksheet.mergeCells(1, 3, 1, 4); 

    // Merge verticaux pour les colonnes qui n'ont pas de label en ligne 1
    // (Sté, ID, Campagne, et les colonnes de fin)
    const fixedCols = [1, 2, 5]; // Sté, ID, Campagne
    fixedCols.forEach(col => {
        worksheet.mergeCells(1, col, 2, col);
    });

    // Merge verticaux pour les colonnes de fin (Jours, Assiduité, etc.)
    const startEndCols = 5 + numDays + 1;
    for (let i = 0; i < 6; i++) {
        const colIndex = startEndCols + i;
        worksheet.mergeCells(1, colIndex, 2, colIndex);
    }

    // --- 4. BOUCLE SUR LES DONNÉES (Corps du tableau) ---
    for (let r = 0; r < rowCount; r++) {
        ...
        for (let c = 0; c < rowData.length; c++) {
            ...
            // Tes règles de couleurs existantes
            if (value === 'A') { bgColor = 'FF0000'; textColor = 'FFFFFF'; }
            else if (value === 'SB') { bgColor = '800080'; textColor = 'FFFFFF'; }
            else if (value === 'C') { bgColor = '90EE90'; }
            else if (value === 'M') { bgColor = 'E0B0FF'; }
            else if (value === 'D') { bgColor = '000000'; textColor = 'FFFFFF'; }
            else if (c >= 5 && c < (5 + numDays)) {
                const dayNum = c - 4;
                const date = new Date(year, month - 1, dayNum);
                const dayOfWeek = date.getDay();
                if (dayOfWeek === 0) { bgColor = 'F4B083'; textColor = '991B1B'; }
                else if (dayOfWeek === 6) { bgColor = '97CDFF'; textColor = '475569'; }
            }

            excelCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF' + bgColor } };
            excelCell.font = { color: { argb: 'FF' + textColor } };
            excelCell.border = { top: {style:'thin'}, left: {style:'thin'}, bottom: {style:'thin'}, right: {style:'thin'} };
            excelCell.alignment = { horizontal: 'center' };
        }
    }

    // Figer les headers (maintenant 2 lignes) et les infos agents
    worksheet.views = [{ state: 'frozen', xSplit: 5, ySplit: 2 }];

    // Téléchargement
    const buffer = await workbook.xlsx.writeBuffer();
    saveAs(new Blob([buffer]), `Export_Pointage_${month}_${year}.xlsx`);
}
